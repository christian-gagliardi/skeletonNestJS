...
import { StoreService } from 'src/entities/stores/services/store.service';
...
describe('Store', () => {
    ...
    it('/Get /details/:id', async()=>{
        const response = await request(app.getHttpServer()).get('/api/store/details/5f5a4faf51853f00188a333b');
        expect(response.statusCode).toBe(200);
    ...
    it('/Get /byDistance/:latitude/:longitude/:skip/:limit', async()=>{
        const response = await request(app.getHttpServer()).get('/api/store/byDistance/42.44786/14.20262/0/5');
        expect(response.statusCode).toBe(200);
        expect(Array.isArray(response.body));
        expect(response.body).toHaveLength(5);
    ...
    it('/Get /byCityAndName/:city/:name', async()=>{
        const response = await request(app.getHttpServer()).get('/api/store/byCityAndName/pescara/esposito');
        expect(response.statusCode).toBe(200);
        expect(Array.isArray(response.body))
    ...
    it('/Put /:id', async()=>{
        const updateR = await request(app.getHttpServer()).put('/api/store/5f5a4faf51853f00188a333b').send({city: 'PESCARa'});
        expect(updateR.statusCode).toBe(200);

        const storeUpdated = await request(app.getHttpServer()).get('/api/store/details/5f5a4faf51853f00188a333b');
        ...
        const updateStore2R = await request(app.getHttpServer()).put('/api/store/5f5a4faf51853f00188a333b').send({city: 'Pescara'});

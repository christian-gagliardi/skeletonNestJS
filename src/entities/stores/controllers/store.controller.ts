...
export class StoreController {
  ...
  @Get('/details/:id')
  getStoreDetails(@Param('id') id:StoreByIdInterface): Promise<StoreInterface>{
    try{
  ...
  async getStoresByCityAndName( @Param('city') city:string, @Param('name') name:string ): Promise<StoreInterface[]>{
    try{
  ...
  @Put('/:id')
  updateStore(@Param('id') id:StoreByIdInterface, @Body() qrInfos: UpdateStoreDto ): Promise<any> {
    try{
      return this.storeService.updateStore(id, qrInfos);
    }catch(err){
      throw new HttpExceptionFilter({message: err})
    }
  }
